...
<div id="carouselExampleControls" class="carousel carousel-dark slide" data-bs-ride="carousel">
  ...
  <?php
    ...
    $sql = "SELECT ISBN,img,title FROM `book`WHERE `Avalibale`=0 ORDER BY RackNumber DESC LIMIT 6;";
    $result = $db->query($sql);
    $checker_two = 0;
    $checker_first = True;

    $sql_c1 = "SELECT a.ISBN FROM reserve a WHERE a.kfupm_id = '$s_kid' AND a.Wait_Position IN( SELECT MIN(b.Wait_Position) FROM reserve b WHERE a.ISBN = b.ISBN AND b.ISBN IN( SELECT c.ISBN FROM book c WHERE c.Avalibale =0 ));";
    $noti = $db->query($sql_c1);
    if(mysqli_num_rows($noti)>0){
      echo '<script type = "text/javascript"> alert ("You have an update in the reservation section!") </script>'; 
    }
...
<div style="background-color:green;">
    <!--First Slider-->
    <h2 style="color:white;">Math Books!</h2>
    <section>
    <div id="carouselExampleControls2" class="carousel carousel-success slide" data-bs-ride="carousel">
    <div class="carousel-inner">
    <?php
    $user = 'root';
    $pass = '';
    $db = 'library1';
    $db = new mysqli('localhost',$user,$pass,$db) or die("Unable to connect");
    $sql = "SELECT DISTINCT ISBN,img,title FROM book NATURAL JOIN subject WHERE `Avalibale`=0 and subject.Category = 'Math';";
    $result = $db->query($sql);
    $checker_two = 0;
    $checker_first = True;
    ...
    while($row = $result->fetch_assoc()) {
      ...
      echo "<div class=\"card\" style=\"background-color:green;\">";
      echo "<div class=\"img-container center\" style=\"background-color:green;\">";
      echo "<img src=".$row["img"]." class=\"card-img-top \" alt=\"book image\" style=\"background-color:green;\">";
      echo "</div>";
      echo "<div class=\"card-body \" style=\"background-color:green;\"> ";
      echo "<h5 class=\"card-title\" style=\"color:white;\">".$row["title"]."</h5>";
       //Borrow form
       echo "<form action=\"\" method=\"get\">";
       echo "<button name=\"Borrow\"type=\"submit\" value=\"".$row["ISBN"]."\" class=\"btn btn-success \">Borrow this book!</button>";
       echo "</form>";
...
<div id="carouselExampleControls3" class="carousel carousel-dark slide" data-bs-ride="carousel">
  ...
  <?php
    ...
    $db = 'library1';
    $db = new mysqli('localhost',$user,$pass,$db) or die("Unable to connect");
    $sql = "SELECT DISTINCT ISBN,img,title FROM book NATURAL JOIN subject WHERE `Avalibale`=0 and subject.Category = 'History';";
    $result = $db->query($sql);
    $checker_two = 0;
    $checker_first = True;
...
<div style="background-color:green;">
    <!--First Slider-->
    <h2 style="color:white;">Not Avaliable!</h2>
    <section>
    <div id="carouselExampleControls4" class="carousel carousel-success slide" data-bs-ride="carousel">
    <div class="carousel-inner">
    <?php
    $user = 'root';
    $pass = '';
    $db = 'library1';
    $db = new mysqli('localhost',$user,$pass,$db) or die("Unable to connect");
    $sql = "SELECT DISTINCT ISBN, img,title FROM book  WHERE  book.Avalibale=1 ;";
    $result = $db->query($sql);
    $checker_two = 0;
    $checker_first = True;
...
<?php
      if(isset($_GET['Borrow'])){
        ...
        location.replace("admin-page.php"); </script>';
